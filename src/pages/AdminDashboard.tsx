...
const AdminDashboard = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-[#020617] text-slate-900 dark:text-slate-300 transition-colors duration-300">
      {/* Tactical Header */}
      <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 sticky top-0 z-50 shadow-sm">
        <div className="max-w-[1800px] mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="font-black text-2xl tracking-tighter text-slate-900 dark:text-white">CITYCARE <span className="text-emerald-500">COMMAND</span></h1>
          ...
          <div className="flex items-center gap-8">
            <div className="hidden xl:flex items-center gap-6">
              <div className="flex items-center gap-3 bg-slate-100 dark:bg-slate-800/50 px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700/50">
                <Server className="w-4 h-4 text-emerald-500" />
                <div className="text-[10px] font-black">
                  <p className="text-slate-500 uppercase">System Load</p>
                  <p className="text-slate-900 dark:text-white">12.4%</p>
                </div>
              </div>
              <div className="flex items-center gap-3 bg-slate-100 dark:bg-slate-800/50 px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700/50">
                ...
                <div className="text-[10px] font-black">
                  ...
                  <p className="text-slate-900 dark:text-white">42ms</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <Button onClick={() => { localStorage.removeItem('current_user'); navigate('/login'); }} variant="ghost" className="rounded-xl font-black text-slate-500 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800">LOGOUT</Button>
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="col-span-12 lg:col-span-3 space-y-6">
          <div className="grid grid-cols-1 gap-4">
            {[
              ...
              <Card key={i} className="bg-white dark:bg-slate-900/50 border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-2xl rounded-3xl overflow-hidden">
          ...
          <Card className="bg-white dark:bg-slate-900/50 border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-2xl rounded-3xl overflow-hidden">
            ...
            <CardContent className="p-4">
              <div className="space-y-3 font-mono text-[10px]">
                {liveLogs.map((log, i) => (
                  <div key={i} className="flex gap-2 text-slate-600 dark:text-slate-400 border-l-2 border-slate-200 dark:border-slate-800 pl-3 py-1">
                    <span className="text-emerald-600 dark:text-emerald-500 shrink-0">[{new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}]</span>
        ...
        <div className="col-span-12 lg:col-span-6 space-y-6">
          <Tabs defaultValue="management" className="w-full">
            ...
            <TabsContent value="management" className="space-y-6">
              <div className="flex flex-col md:flex-row gap-4 items-center">
                <div className="relative flex-1 w-full">
                  ...
                  <Input placeholder="Search Tactical Database..." className="pl-12 h-14 rounded-2xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 text-slate-900 dark:text-white font-bold placeholder:text-slate-400 dark:placeholder:text-slate-600" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
                </div>
                <select className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-2xl px-6 h-14 text-xs font-black text-slate-600 dark:text-slate-400 shadow-sm outline-none focus:ring-2 focus:ring-emerald-500 w-full md:w-auto" value={statusFilter} onChange={e => setStatusFilter(e.target.value)}>
              ...
              <div className="grid gap-6 overflow-y-auto max-h-[calc(100vh-350px)] pr-2 custom-scrollbar">
                {filteredIssues.map(issue => (
                  <Card key={issue.id} className="bg-white dark:bg-slate-900/80 border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-2xl rounded-[2.5rem] overflow-hidden group hover:border-emerald-500/30 transition-all duration-500">
                    <CardContent className="p-8">
                      <div className="flex flex-col xl:flex-row gap-8">
                        <div className="flex-1 space-y-6">
                          <div className="flex items-center gap-3 flex-wrap">
                            <Badge className={`rounded-lg px-3 py-1 font-black text-[10px] uppercase tracking-widest ${issue.priority === 'High' ? 'bg-rose-500/20 text-rose-600 dark:text-rose-500 border border-rose-500/30' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'}`}>
                              {issue.priority} PRIORITY
                            </Badge>
                            <Badge className={`rounded-lg font-black text-[10px] uppercase tracking-widest ${issue.status === 'Resolved' ? 'bg-emerald-500/20 text-emerald-600 dark:text-emerald-500 border border-emerald-500/30' : issue.status === 'Completed' ? 'bg-sky-500/20 text-sky-600 dark:text-sky-500 border border-sky-500/30' : 'bg-amber-500/20 text-amber-600 dark:text-amber-500 border border-amber-500/30'}`}>
                              {issue.status}
                            </Badge>
                            <span className="text-[10px] font-mono text-slate-400 dark:text-slate-600 ml-auto">ID: {issue.id}</span>
                          </div>
                          
                          <div>
                            <h3 className="font-black text-2xl text-slate-900 dark:text-white tracking-tight mb-2">{issue.title}</h3>
                            <p className="text-slate-600 dark:text-slate-400 font-medium leading-relaxed text-sm">{issue.description}</p>
                          </div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
                            {issue.imageUrl && (
                              <div className="space-y-2">
                                ...
                                <div className="aspect-video rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950">
                            ...
                            {issue.workerReport?.imageUrl && (
                              <div className="space-y-2">
                                ...
                                <div className="aspect-video rounded-2xl overflow-hidden border border-emerald-500/20 bg-slate-50 dark:bg-slate-950">
                          ...
                          <div className="bg-slate-50 dark:bg-slate-800/30 p-6 rounded-3xl border border-slate-200 dark:border-slate-700/30 space-y-4">
                            {issue.status === 'Pending' && (
                              <div className="space-y-3">
                                ...
                                <select 
                                  className="w-full h-12 rounded-xl border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-bold px-4 text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-emerald-500"
                            ...
                            {issue.status === 'In Progress' && (
                              <div className="text-center py-4 space-y-3">
                                ...
                                <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Personnel On-Site</p>
                                <p className="text-xs font-bold text-slate-900 dark:text-white">{workers.find(w => w.id === issue.workerId)?.name}</p>
                              </div>
                            )}

                            {issue.status !== 'Resolved' && issue.status !== 'Flagged' && (
                              <Button variant="outline" className="w-full rounded-xl h-12 font-black border-slate-200 dark:border-slate-700 text-rose-600 dark:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 hover:border-rose-500/50" onClick={() => handleStatusUpdate(issue.id, 'Flagged')}>
                  ...
                  <div className="h-[300px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        ...
                        <Tooltip contentStyle={{ backgroundColor: '#0f172a', border: 'none', borderRadius: '12px', fontSize: '12px', color: '#fff' }} />
                        <Legend verticalAlign="bottom" height={36} wrapperStyle={{ paddingTop: '20px', fontSize: '10px', fontWeight: 'bold', textTransform: 'uppercase', color: '#94a3b8' }} />
                      </PieChart>
                    </ResponsiveContainer>
                  </div>
                </Card>
            ...
            <TabsContent value="moderation" className="space-y-6">
              <Card className="bg-white dark:bg-slate-900/50 border-slate-200 dark:border-slate-800 rounded-[2.5rem] overflow-hidden">
                  ...
                  {reportedIssues.length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {reportedIssues.map(issue => (
                        <div key={issue.id} className="flex items-center justify-between p-6 bg-slate-50 dark:bg-slate-800/30 rounded-3xl border border-slate-200 dark:border-slate-700/30">
                          <div className="space-y-1">
                            <p className="font-black text-slate-900 dark:text-white">{issue.title}</p>
                            <p className="text-xs text-rose-600 dark:text-rose-500 font-bold uppercase tracking-widest">{issue.reports.length} Security Flags Raised</p>
                          </div>
                          <div className="flex gap-3">
                            <Button variant="outline" className="rounded-xl border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white" onClick={() => issueService.dismissReports(issue.id)}>DISMISS</Button>
              ...
              {mockDb.users.filter(u => u.role === 'citizen').sort((a, b) => (b.points || 0) - (a.points || 0)).slice(0, 5).map((u, i) => (
                <div key={u.id} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center font-black text-[10px] text-emerald-600 dark:text-emerald-500 border border-emerald-500/20">
                      {i + 1}
                    </div>
                    <span className="font-bold text-xs text-slate-700 dark:text-slate-300">{u.name}</span>
                  </div>
                  <Badge variant="outline" className="font-black text-[10px] border-emerald-500/20 text-emerald-600 dark:text-emerald-500">{u.points || 0} PTS</Badge>
                </div>
              ))}
            </CardContent>
          </Card>
      ...
      {/* Tactical Footer */}
      <footer className="bg-white dark:bg-slate-900/80 border-t border-slate-200 dark:border-slate-800 py-12 px-8 mt-auto">
        <div className="max-w-[1800px] mx-auto flex flex-col md:flex-row justify-between items-center gap-8 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">
          ...
          <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-800/30 px-6 py-3 rounded-2xl border border-slate-200 dark:border-slate-700/30">
            <Code2 className="w-4 h-4 text-emerald-500" />
            <span>Built by <span className="text-emerald-600 dark:text-emerald-500">Deploy & Pray</span></span>
          </div>

          <div className="flex items-center gap-3">
            <span className="text-slate-400 dark:text-slate-600">Network Node: v4.2.0-Stable</span>
            <div className="h-4 w-px bg-slate-200 dark:bg-slate-800" />
            <span className="text-emerald-600/50 dark:text-emerald-500/50">© 2024 CityCare Command</span>
          </div>
        </div>
      </footer>
    </div>
